...
def call_reasoning_model(user_query: str, messages: Optional[List[BaseMessage]] = None) -> ToolResponse:
    """Resolve reasoning query."""
    reasoning_model_name = "gpt-3.5-turbo"
    reasoning_model_response = call_openai(model_name=reasoning_model_name, system_msg=REASONING_PROMPT, human_msg=user_query, messages=messages)
    reasoning_model_response.response_generated_via = reasoning_model_name
    return reasoning_model_response

def call_coding_model(user_query: str, messages: Optional[List[BaseMessage]] = None) -> ToolResponse:
    """Resolve coding query."""
    coding_model_name = "claude-opus-4-1-20250805"
    coding_model_response = call_anthropic(model_name=coding_model_name, system_msg=CODING_PROMPT, human_msg=user_query, messages=messages)
    coding_model_response.response_generated_via = coding_model_name
    return coding_model_response

def call_image_generation_model(user_query: str, messages: Optional[List[BaseMessage]] = None) -> ToolResponse:
    ...
    image_generation_model_response = call_gemini_image_generation_model(model_name=image_generation_model_name, system_msg=IMAGE_GENERATION_PROMPT, human_msg=user_query, messages=messages)
    image_generation_model_response.response_generated_via = image_generation_model_name
    return image_generation_model_response

def call_realtime_info_model(user_query: str, messages: Optional[List[BaseMessage]] = None) -> ToolResponse:
    """Fetch real time info."""
    realtime_model_name = "sonar-pro"
    realtime_model_response = call_perplexity_api(model_name=realtime_model_name, system_msg=REAL_TIME_INFO_PROMPT, human_msg=user_query, messages=messages)
...
def call_sarvam_model(user_query: str, messages: Optional[List[BaseMessage]] = None) -> ToolResponse:
    """Fetch real time info."""
    sarvam_model_name = "sarvam-v2"
    sarvam_model_response = call_sarvam(user_query=user_query, messages=messages)
    sarvam_model_response.response_generated_via = sarvam_model_name
    return sarvam_model_response

def call_default_model(user_query: str, messages: Optional[List[BaseMessage]] = None) -> ToolResponse:
    """Call the default model."""
    default_model_name = "gpt-5-nano"
    default_model_response = call_openai(model_name=default_model_name, system_msg=DEFAULT_PROMPT, human_msg=user_query, messages=messages)
...
def call_model_by_name(model_name: str, user_query: str, messages: Optional[List[BaseMessage]] = None) -> ToolResponse:
    ...
    if model_provider == ModelProvider.OPEN_AI:
        return call_openai(model_name=model_name, system_msg=DEFAULT_PROMPT, human_msg=user_query, messages=messages)
    elif model_provider == ModelProvider.ANTHROPIC:
        return call_anthropic(model_name=model_name, system_msg=DEFAULT_PROMPT, human_msg=user_query, messages=messages)
    elif model_provider == ModelProvider.PERPLEXITY:
        return call_perplexity(model_name=model_name, system_msg=DEFAULT_PROMPT, human_msg=user_query, messages=messages)
    elif model_provider == ModelProvider.X_AI:
        return call_grok(model_name=model_name, system_msg=DEFAULT_PROMPT, human_msg=user_query, messages=messages)
    elif model_provider == ModelProvider.GEMINI:
        return call_gemini(model_name=model_name, system_msg=DEFAULT_PROMPT, human_msg=user_query, messages=messages)
    elif model_provider == ModelProvider.SARVAM:
        return call_sarvam(user_query=user_query, messages=messages)